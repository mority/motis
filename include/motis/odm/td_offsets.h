...
nigiri::routing::td_offsets_t get_td_offsets(auto const& rides,
  ...
  utl::equal_ranges_linear(
      ...
      [&](auto&& from_it, auto&& to_it) {
        ...
        for (auto const& r : nigiri::it_range{from_it, to_it}) {
          auto const tdo = nigiri::routing::td_offset::make(
              std::min(r.time_at_stop_, r.time_at_start_),
              std::chrono::abs(r.time_at_stop_ - r.time_at_start_), mode);
          ...
          if (i + 1 == end(td_offsets.at(r.stop_)) ||
            ...
            td_offsets.at(r.stop_).insert(
                i + 1, nigiri::routing::td_offset::make(
                           tdo.valid_from_ + 1min,
